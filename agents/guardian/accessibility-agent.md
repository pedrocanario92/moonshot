...
First operational spec for the Guardian Layer concept introduced in `moonshot.html` (Stage 3 — "Accessibility Agent — WCAG compliance"). That card had no spec until this work. Scope intentionally narrow (Warehouse homepage only, Direction A single-focus stage layout) to establish the pattern. CLI / Map / Log / Perf views are out of scope for v1.
