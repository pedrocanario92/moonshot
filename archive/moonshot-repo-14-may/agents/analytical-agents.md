...
- Surfacing analytical findings to the Archer Chat Agent for user review.
...
- User communication routing and approval gate — owned by the Archer Chat Agent.
...
- The Archer Chat Agent dispatches the agent based on user intent within analysis mode.
...
- Emit analytical findings to the Archer Chat Agent for surfacing to the user.
...
- Application of a handoff summary to a newly activated analytical agent must always be confirmed by the user — the user reviews and may edit the summary before confirming. The Archer Chat Agent enforces this gate on behalf of the analytical agents.
...
- Archer Chat Agent: activation, dispatch within analysis mode, handoff summary on agent switch, deactivation on mode exit.

### Sends signals to
- Archer Chat Agent: analytical findings; handoff summary on agent switch (generated for the user to review and edit before confirmation).

### Prohibited communication
- The analytical agents must not communicate directly with any domain sub-agent (Pick Path, Labor, Order Priority, Carrier, Exception). All routing is via the Archer Chat Agent.
...
- The analytical agents must not communicate with each other directly. Agent switching is mediated by the Archer Chat Agent, with the user reviewing and confirming the handoff summary.
...
- The analytical agents must not address the human user directly outside of analysis mode — all routing is via the Archer Chat Agent.
...
- The analytical agents must always propose handoff summaries to the user (via the Archer Chat Agent) and never load a new analytical agent with an unconfirmed summary.
...
- Handoff summary generation is Tier 3, but application of the summary to the new agent is Tier 2 and gated by the Archer Chat Agent.

---

## 7. Escalation Behaviour

If user-provided data is malformed, ambiguous, or insufficient for the agent's reasoning mode, the analytical agent must escalate to the Archer Chat Agent with an explicit data-quality failure signal and must not interpolate or guess. If a WMS read API fetch fails (data unavailability), the analytical agent must escalate to the Archer Chat Agent and must not present a finding based on stale or partial data.

If two analytical findings within the same agent's mode conflict (for example: BI finds two contradictory trends in the same data window), both must be surfaced to the Archer Chat Agent with attribution — the analytical agent must not silently reconcile.

If the user requests an analysis that falls outside the active analytical agent's reasoning mode (for example: the user asks the BI Agent for a causal explanation), the agent must surface the mode mismatch to the Archer Chat Agent so the user can choose an agent switch — the agent must not silently shift reasoning mode.

If a handoff summary cannot be generated (for example: prior context is unavailable or corrupt), the analytical agent must escalate to the Archer Chat Agent and must not load the new agent's session — a blind handoff is a trust failure.
