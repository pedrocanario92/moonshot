...
**Added 2026-05-13.** The Order Priority Agent operates in two product contexts. In WMS topology it tracks SLA clocks on orders already released to the warehouse floor — the at-risk-window detection runs against pick progress, wave status, and outbound dispatch readiness. In OM topology the same agent tracks SLA clocks on orders upstream — at the moment of channel intake, allocation, and tender — before they ever reach a warehouse. The agent's Tier 1 prohibition on cancellation, the Tier 2 obligation to propose-not-execute reprioritisation, and the three shared fields (`confidence`, `escalationReason`, `options[]`) are identical across both contexts. Joint recommendations with Domain agents in the OM topology (Allocation Agent, Cutoff Manager Agent) follow the same Lead-Agent-routed pattern as joint recommendations with WMS Domain agents.
...
- Order reprioritisation proposals — the Order Priority Agent must always propose, never execute, without explicit human confirmation routed through the Archer Chat Agent.
- SLA-driven labor redeployment proposals built with the Labor Agent must always be routed through the Archer Chat Agent for consolidated user confirmation.
- SLA-driven carrier reroute or window-extension proposals built with the Carrier Agent must always be routed through the Archer Chat Agent for consolidated user confirmation.
...
### Sends signals to
- Archer Chat Agent: Tier 2 proposals (order reprioritisation, SLA-driven labor redeployment, SLA-driven carrier reroute or window extension); escalation requests when scope exceeded.
...
Joint recommendations must be assembled with both agents' contributions tagged and routed through the Archer Chat Agent for consolidated user proposal. The Order Priority Agent must not present a joint recommendation directly to the user.
...
- The Order Priority Agent must not address the human user directly — all user-facing communication routes through the Archer Chat Agent.
...
- The Order Priority Agent must always propose SLA-driven labor redeployments through the Archer Chat Agent and never execute them on the strength of an internal coordination response from the Labor Agent.
- The Order Priority Agent must always propose SLA-driven carrier reroutes or window extensions through the Archer Chat Agent and never execute them on the strength of an internal coordination response from the Carrier Agent.
...
If order data, ship-by windows, or pick / pack progress data are unavailable, the Order Priority Agent must escalate to the Archer Chat Agent with an explicit data-unavailability signal and must not propose reprioritisations based on stale data.

If the Labor Agent or Carrier Agent does not respond to a coordination signal within the configured window, the Order Priority Agent escalates to the Archer Chat Agent so the orchestrator can dispatch directly. If the Labor Agent and Carrier Agent return conflicting candidate responses to the same SLA risk (for example: Labor proposes a redeployment that resolves the risk while Carrier proposes a window extension that resolves it differently), both responses must be surfaced to the Archer Chat Agent with attribution — the Order Priority Agent must not silently choose one.

If a candidate reprioritisation would breach a Tier 1 prohibition (creating a new at-risk order or implying cancellation of an existing order), the agent must discard the proposal and escalate the underlying SLA risk to the Archer Chat Agent without proposing the reprioritisation.
