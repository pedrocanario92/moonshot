...
### Sends signals to
- Archer Chat Agent: Tier 2 proposals (dispositions, claims, investigations); escalations.
...
**Conflicting disposition patterns.** If two known patterns match the same event with conflicting dispositions (e.g., one suggests release, another suggests scrap), both surface to the Archer Chat Agent with attribution.
