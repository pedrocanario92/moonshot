...
## May 14 2026 — Deployment options section added — Guardian agents shippable as runtime or SDLC skills.

_May 14 2026 — UX governance reframed as a deployable function, not just a runtime layer._

### What was built

- New section `#section-deployment` on The System page (`view-system`), positioned between `#section-topology` (Communication Topology) and `#section-case-system` (the case closing). Two-column presentation:
  - **Option A · Runtime agents** — Guardian Agents (Heuristics, Accessibility) running alongside the rest of the agent system, auditing outputs in operation, surfacing violations through the same approval channels as domain agents, logging into the trust calibration loop. Catches drift and failure modes that only appear at runtime.
  - **Option B · Skills and SDLC checks** — governance encoded as Claude skills (UI kit, WCAG 2.2 AA, Nielsen Norman 10), invoked during agent prototyping so output is constrained at generation time. Dev-time linters and CI checks block violations before merge. Lower runtime cost, faster developer feedback loop.
- Closing paragraph anchors the duality: *"Most production deployments will use both. Design-time constraints catch what's catchable early; runtime auditing handles what only shows up in operation. UX orchestration covers both layers."*
- Soft attribution line below: *"Both deployment models are endorsed by Infios AI engineering."* Deliberately company-level — no individuals named.
- The System sticky index updated to: Architecture · Pillar Timelines · System Tree · Topology · **Deployment options** · The case. `wireIndex` array on `view-system` extended to include `section-deployment` so scroll-highlight picks it up.
- The Agents page Guardian cards (Accessibility Agent, Heuristics Agent) each carry a new dashed-rule cross-reference line near the bottom of the card body, between the `card__field` block ("Talks to") and `card__guardrail`: *"Deployable as a runtime agent or as a Claude skill at design time — see The System → Deployment options."* Link uses the existing `data-go="system"` cross-view navigation pattern.
- The Home page Guardrails value prop (`.value-prop` card #3 of 4) gets one trailing sentence: *"Specs deploy as runtime agents or as design-time SDLC checks — the same governance, two paths into the stack."* Other three cards (Oversight, Intervention, Cohesion) unchanged.
- CSS added in a new `DEPLOYMENT OPTIONS (System page)` block before the `ICONS` section, plus a small `AGENT CARD: cross-reference line` block right after it. Tokens used: `--surface-base` (section background), `--surface-tinted` (card background — same neutral tint for both options so they read as parallel), `--border-hairline`, `--charcoal` (left border on Option A — runtime), `--accent-deep` (left border on Option B — skills), `--text-primary/secondary/tertiary`. No blue anywhere.

### Why

- The runtime-only framing of UX governance was incomplete. A meaningful portion of the governance is enforceable at design time through Claude skills and dev-time CI checks — catching violations before they ship, not auditing them after. The page implied otherwise.
- Presenting both options as parallel paths reframes UX orchestration as flexible deployment infrastructure rather than a single product. Most real deployments use both layers — design-time constraints + runtime auditing — and the page now says so explicitly.
- The duality strengthens the case for UX orchestration's role across the agentic SDLC — spec authoring through dev-time enforcement through runtime auditing — rather than narrowing it to runtime governance alone.

### Verified (via `preview_eval`)

- Section structure: `#section-deployment` renders with 2 `.deployment-option` cards, 10 list items total (5 per card), closing paragraph, attribution line.
- Desktop (1280px): grid template `548px 548px`, both cards equal width and equal height (353px each on the 1280 test).
- Mobile (375px): grid collapses to single column; both cards stack at 335px wide.
- Intermediate (880px breakpoint): single column kicks in at and below 880, two columns at 900 (cards 406.5px each — tight but legible).
- Sticky index now returns 6 entries in order: Architecture · Pillar Timelines · System Tree · Topology · **Deployment options** · The case. Clicking the Deployment options chip scrolls the section into view (`getBoundingClientRect().top ≈ 96`) and the chip gets `is-active`.
- Guardian section returns 2 `.card__cross-ref-line` elements (one per Guardian card), each linking to `#system` with `data-go="system"`.
- Home value props in order — Oversight, Intervention, **Guardrails** (body now ends *"…the same governance, two paths into the stack."*), Cohesion. Other three card bodies are unchanged.
- Contrast (WCAG AA threshold: 4.5:1 for body, 3:1 for large text): section-label 6.13, section lede 10.38, deployment-option label 5.87, deployment-option title 15.91, list items 9.95, closing paragraph 16.61, attribution (italic 13px) 6.13. All pass.
- No console errors. No duplicate IDs (`document.querySelectorAll('[id]')` scan returned no duplicates).
- Pre-edit snapshot saved to `archive/moonshot-home.PRE-DEPLOYMENT-OPTIONS.html`.

### What it leaves

- The Agents page Guardian section continues to describe WHAT the agents do; The System page Deployment options section describes HOW they ship. Different scope, complementary content — readers who land on either page get pointed at the other.
- The Home page now reflects the full picture: UX orchestration covers spec authoring (Architecture conveyor station 1), dev-time enforcement (new Option B), and runtime auditing (existing Guardian stations 5–6) as three layers of the same function — Guardrails on the Home page is the entry point to that arc.
- The deployment-options framing is groundwork for future content about the agentic SDLC pipeline if Infios formalises that concept; the language ("skills," "SDLC checks," "linters before merge") is in place to be expanded.

### Caveats reported

- **Intermediate width (~900–1100px)**: the two cards at 900px are ~407px each — content is legible but tight (5 list items, headline, label). Could justify either raising the single-column breakpoint to ~1000px or adding `grid-template-columns:repeat(auto-fit, minmax(360px, 1fr))` for a softer transition. Left as a fixed two-column-above-880 / single-below-880 binary for now — matches the existing site's responsive pattern (the closing pull-quote `.value-grid` and the `.grid-3` blocks also use fixed breakpoints rather than `auto-fit`).
- **Guardian card layout**: the new `card__cross-ref-line` slotted between `card__field` and `card__guardrail` without restructuring — the existing card body is a vertical stack of opaque blocks, so adding one more dashed-rule row was a clean fit. Both Guardian cards still render at equal height in the `grid-3`.
- **Contrast tweaks**: none needed. All seven inspected text/background pairs cleared AA on the first pass with the existing token palette (`--text-primary/secondary/tertiary` against `--surface-base` and `--surface-tinted`).
- **Cross-view deep linking**: clicking the cross-reference line from a Guardian card switches to The System view but lands at the top of the page (the existing `showView` handler does `window.scrollTo(0,0)` and `preventDefault`s the anchor). Users then click "Deployment options" in the sticky index to jump to the section. Considered extending `showView` to honour the href hash after the switch but chose not to — would change the behaviour of every existing `data-go` link on the site (including the home page evidence cards, which intentionally scroll to top). Out of scope for this pass; flag for future if cross-view deep linking becomes a recurring need.
- **`.claude/launch.json` correction**: the launch.json was pointing to `moonshot-repo-13-may` (yesterday's working dir). Corrected to `moonshot-repo-14-may` so `preview_start` serves the right file. Unrelated to the content change but blocked verification until fixed.
- `preview_screenshot` timed out (same recurring symptom — verification done via `preview_eval` text introspection, contrast math, and `preview_console_logs`).
