...
│                                                                     │
│  Archer Chat Agent (orchestrator)                                   │
...
│                                                                     │
│  WMS-pillar agents shown above. TM and OM pillars add 4 more        │
│  Domain agents (Spot Rate, Route, Allocation, Cutoff Manager) —     │
│  see §6.6 for cross-pillar treatment.                               │
│                                                                     │
...
### 4.1 The Archer Chat Agent is an orchestrator — not a domain expert

**Decision:** The Archer Chat Agent routes intent, manages conversation state, dispatches to sub-agents, and enforces approval gates. It does not monitor the warehouse or propose actions.
...
**Decision:** A dedicated Shift Intelligence Agent monitors all zones continuously, detects anomalies, and signals the Archer Chat Agent. It does not propose actions or receive approvals. It feeds everything else.
...
### 4.3 High-volume e-commerce agent roster — eight WMS domain agents
...
**Cross-pillar expansion (as of May 13 afternoon):** The eight-agent roster above is WMS-scoped. Four additional Domain agents now exist in adjacent pillars — **Spot Rate** and **Route** (TM); **Allocation** and **Cutoff Manager** (OM) — and two existing WMS agents (Carrier, Order Priority) now operate across pillars under the dual-context pattern. The cross-pillar layer is documented in §6.6. The WMS-only count of eight remains correct for the high-volume e-commerce archetype; cross-pillar counts (twelve Domain agents in total) only apply when a customer runs more than one Infios product.
...
**Where it lives:** The sim engine lives in `moonshot-prototype.html` and must never appear inside any shipped homepage variant of the deliverable. The spec for the sim engine lives at `agents/meta/warehouse-life-agent.md`.
...
**Decision:** Every proposed action is tagged `urgent` or `standard` at generation time by the Archer Chat Agent. This tag drives the entire escalation path.
...
### 6.5 Cross-pillar architecture

**Decision:** The prototype is a multi-product container, not a single-product app. A `currentProduct` state variable governs which pillar is mounted: `'wa'` (Warehouse Advantage), `'tm'` (Transportation Management), `'om'` (Order Management), or `'cross'` (the multi-product roll-up). UI surfaces split cleanly into two tiers — **agnostic** components that exist for every pillar, and **domain-specific** components that swap with `currentProduct`.

**Driver:** Richard Stewart's May 12, 2026 directive: pillar-specific demos first (WMS, TMS, OMS) alongside the existing WMS work, common agnostic components, domain-specific tiles per product, and a cross-pillar dashboard "for what it looks like when customers are running more than one." Gartner presentation in ~16 days — the cross-pillar story must hold on stage, not just in the deck.

**The agnostic / domain split:**

| Tier | Components | Behaviour |
|---|---|---|
| **Agnostic** (shared across WA · TM · OM · cross) | Dashboard slot · Archer chat · Agent Performance · Activity Log pattern · Decision Queue concept | Visual language, layout grid, and interaction patterns are identical across products. Content is sourced from whichever pillar's data object is active. |
| **Domain-specific — WA** | Floor Map · Shift Log · Warehouse view · Zones strip | Sourced from `WAREHOUSE` state object. Driven by the Warehouse Life Agent's sim engine. |
| **Domain-specific — TM** | Network Map · TM Activity Log (lanes) · Lanes strip · Carrier scenarios | Sourced from `TM_DATA`. Static demo data — no sim engine. |
| **Domain-specific — OM** | Order Dashboard (pipeline) · Order Log · Channels strip · Stock-out / cutoff scenarios | Sourced from `OM_DATA`. Static demo data — no sim engine. |

**The `currentProduct` state model:**

A single global variable, initialised to `'wa'` so existing prototype behaviour is byte-identical when the page loads. The header product switcher (WA · TM · OM · cross) calls `setProduct(p)` which updates the state, swaps the wordmark and shell title from `PRODUCT_META`, falls back to `view = 'home'` when the current view isn't valid for the new product, and re-renders. `renderMain()` branches on `currentProduct` first, then on `view`. Sim-tick side effects are gated by `currentProduct === 'wa'` so the WA sim engine cannot poke the DOM while a non-WA pillar is mounted.

The sidebar is split into a constant **CROSS-PILLAR** section (Dashboard · Archer · Agent Performance — always visible) and a **domain** section driven by `DOMAIN_NAV[currentProduct]`. Reading the sidebar tells you the architecture without opening a spec.

**The cross-pillar dashboard (`currentProduct === 'cross'`):**

Not a clone of the pillar dashboards. It is a command-centre overview, designed for the operator running more than one Infios product simultaneously. Four bands:

1. **Three product health cards** (WA · TM · OM) — each shows the pillar's headline KPIs, a status indicator, and acts as the navigation entry point into that pillar.
2. **Unified decision queue** — every pending proposal across products, sorted by urgency. Each row carries a product badge sourced from the proposal's `product` field.
3. **Archer terminal** — pre-seeded with messages that name cross-product correlations (e.g., *"Carrier delay on MEM→CHI may impact 12 warehouse outbound orders in the next wave"*). This is the "glass box" — agents explaining relationships across products.
4. **Aggregated KPIs** — totals rolled up across all three pillars.

**The `product` field on agent proposals:**

Every `AgentProposal` carries `product: "wa" | "tm" | "om"`. The cross-pillar dashboard's unified decision queue uses this field to render its product badges and to drive cross-product correlation logic. A TM carrier-delay proposal can carry an `impacts[]` entry whose `area` names a WA outbound wave — the cross-pillar view can then surface both proposals as a linked pair. The schema deliberately omits `'cross'` from the product enum: the cross-pillar dashboard is a view, not a product an agent emits from. Every proposal originates in a single pillar even when its impacts span pillars.

**The machine-readable contract:** `agent-response-schema.json` at the repo root. Authored by the UX team, pre-establishing the format Allen Oleksak's monitoring-agent specs will conform to. Cross-references the three shared patterns above (section 6.2): `confidence`, `escalationReason`, `options[]` — and adds `criticality`, `tier`, `product`, `impacts[]` so the cross-product correlation work has a stable shape from day one.

---

### 6.6 Cross-Pillar Domain Agents

**Decision:** Section 6.5 establishes the cross-pillar **container** — the `currentProduct` state model, the agnostic / domain component split, the cross-pillar dashboard's four bands. This section establishes the cross-pillar **agent layer** that lives inside that container: four new Domain agents native to TM and OM, plus a dual-context pattern that lets a single agent operate inside two pillars without forking the spec.

**Driver:** Richard Stewart's May 12, 2026 directive — already named in §6.5 — called for pillar-first demos with agnostic vs domain-specific components and a cross-pillar dashboard as the multi-product roll-up. The morning of May 13 shipped the container with TM and OM views populated from `TM_DATA` / `OM_DATA`, but the agents the views referenced (Spot Rate, Route, Allocation, Cutoff Manager) had no governance documents underneath them. The May 13 afternoon backfill closes that gap so all three pillars read at the same quality bar as the eight existing WMS specs.

**The four new Domain agents:**

| Agent | Pillar | Spec location | Owns |
|---|---|---|---|
| **Spot Rate Agent** | TM | `agents/spot-rate-agent.md` | Spot quotes vs contracted lane rate. Tier 2 escalation when a spot quote crosses the 2× lane-rate threshold Richard called out by name on May 12. |
| **Route Agent** | TM | `agents/route-agent.md` | Lane health, mode selection (truckload / LTL / intermodal), multi-stop routing. Tier 2 reroute proposals when lane conditions degrade. |
| **Allocation Agent** | OM | `agents/allocation-agent.md` | Constrained-inventory allocation across orders. Tier 2 proposals when demand exceeds supply for a SKU. |
| **Cutoff Manager Agent** | OM | `agents/cutoff-manager-agent.md` | Carrier cutoff windows at the order layer. Tier 2 proposals when orders are at risk of missing a cutoff. |

Each spec follows the same 8-section structure as the WMS specs (Identity, Scope, Triggers, Actions, Communication, Guardrails, Escalation, Metrics) and carries `confidence`, `escalationReason`, `options[]` on every Tier 2 proposal.

**The dual-context pattern:**

Two existing WMS agents now operate across pillars without spec duplication:

- **Carrier Agent** operates in WMS dock-window context AND TM lane-window context. In WMS the operational substrate is dock-door appointments, inbound/outbound window slips, and yard-to-dock sequencing. In TM the operational substrate is lane ETA visibility, carrier ETA-vs-promised drift, and lane-rate breaches. Identical Tier 1/2/3 boundaries across both contexts.
- **Order Priority Agent** operates in WMS floor-released SLA context AND OMS upstream channel-intake context. In WMS the agent reasons over SLA risk for orders already on the pick path. In OMS the agent reasons over SLA risk at order intake — channel cutoffs, promised ship-by windows, reprioritisation across the unfulfilled queue. Identical Tier 1/2/3 boundaries across both contexts.

Only the operational substrate (data feed, scope of triggers, surface where proposals land) changes between contexts. Tier-boundary and escalation-path conventions stay constant. The cross-pillar addenda inside `agents/carrier-agent.md` and `agents/order-priority-agent.md` document each dual-context spec's second context; the base spec covers the WMS context. `agents/archer-chat-agent.md` gained a parallel addendum covering how the Archer Chat Agent routes intent across the three pillars.

**Why dual-context, not separate agents:** Forking a Carrier Agent into "WMS Carrier" and "TM Carrier" would duplicate the reasoning model and split the audit trail. Operators running both products would see two different agent identities making related calls — confusing in the conversation log and brittle when proposals correlate. One agent, two contexts, identical guardrails: same identity in every log entry, single rejection-pattern signal on the Agent Performance page, and cross-context correlation falls out naturally because the `impacts[]` of a proposal can reference the *other* pillar without leaving the agent's scope.

**The `product` discriminator on agent proposals:**

Every `AgentProposal` carries `product: "wa" | "tm" | "om"` so the Archer Chat Agent can correlate proposals across products when a customer runs more than one. This is the same field introduced in §6.5 — re-emphasised here because it is what makes cross-pillar correlation mechanical rather than heuristic. A TM Spot Rate proposal and an OM Cutoff Manager proposal that both reference the same outbound wave can be surfaced as a linked pair on the cross-pillar dashboard's unified queue purely by matching `impacts[].area` across two `product` values. The agents do not need to know about each other — the `product` + `impacts[]` shape carries the correlation.

**Forward reference:** The machine-readable contract for every agent proposal — including the new four — lives at `agent-response-schema.json` at the repo root. The four new specs conform to that schema without exception.
...
| Sub-agent proliferation without routing updates | Low | New agents cannot be added without updating Archer Chat Agent routing logic. Intent black holes otherwise. |
...
| Sim engine leakage into shipped product | Medium | Warehouse Life Agent is a Tier 1 prohibition outside `moonshot-prototype.html`. Verified before every share-out by snapshotting the prior version into `archive/` and grep-checking that no sim-engine code appears in any shipped homepage variant. |
...
**Current key tokens** (as of May 12 morning — definitive values live in the `:root` blocks of `moonshot-home.html` and `moonshot-prototype.html`):
